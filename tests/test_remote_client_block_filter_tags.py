...
@pytest.fixture
def client():
    """MirixClient with mocked async _request."""
...
class TestMirixClientSendMessageBlockFilterTags:
    ...
    @pytest.mark.asyncio
    async def test_send_message_includes_block_filter_tags_in_request(self, client):
        """When block_filter_tags is provided, it is sent in the request body."""
        block_filter_tags = {"env": "staging", "team": "platform"}
        client._request = AsyncMock(return_value={"messages": [], "usage": {}})
    ...
    @pytest.mark.asyncio
    async def test_send_message_without_block_filter_tags(self, client):
...
class TestMirixClientUserMessageBlockFilterTags:
    ...
    @pytest.mark.asyncio
    async def test_user_message_includes_block_filter_tags_in_request(self, client):
        """user_message(block_filter_tags=...) results in block_filter_tags in request."""
        block_filter_tags = {"env": "prod"}
        client._request = AsyncMock(return_value={"messages": [], "usage": {}})

        await client.user_message(
            agent_id="agent-456",
            message="Hi",
            block_filter_tags=block_filter_tags,
        )
    ...
    @pytest.mark.asyncio
    async def test_user_message_passes_block_filter_tags_to_send_message(self, client):
