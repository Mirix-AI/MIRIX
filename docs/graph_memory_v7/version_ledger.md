...
| **D** | **LLM triple extraction** (keep relations, per-chunk, abstraction) — replaces GLiNER as main extractor | G6 cost + concept coverage + relations (for PPR) | **validated** (prompt + example in `extractor_direction_D.md`); build next round; likely becomes v7.6 (merges with keep-relations+PPR) | — | — | — | — |
