...
| **v7.4** | **GLiNER extractor** (local encoder) — dialogue roles (User/Assistant) filtered out | **G6** cost + **G4** noise hubs | **shipped** — graph-build **~200× faster** (98s vs ~5.5h for 962 mem); User/Assistant hubs gone; deterministic | 2473 (−52%) | 66% | **32/60 (vs v7 30/60; +2, within noise, WITH SUPPORTED_BY handicap)** | pending |
