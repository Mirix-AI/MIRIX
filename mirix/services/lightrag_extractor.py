...
def parse_extraction_output(raw: str) -> ExtractionResult:
    ...
    for raw_line in raw.splitlines():
        ...
        if kind == "entity" and len(parts) >= 4:
            ...
            # Relations from the LightRAG format are no longer surfaced — nothing in
            # the live v7/v8 path reads them (relation edges come from the v7.6 triple
            # extractor). Skip instead of accumulating a dead output.
            continue
        else:
...
async def extract_entities_and_relations(
    ...
    *,
    ...
    logger.info(
        "LightRAG extraction: %d entities from %d chars",
        len(parsed.entities),
        len(text),
    )
    return parsed
