...
def parse_distiller_json_array(raw: Optional[str]) -> List[Dict]:
    """Robustly parse a distiller LLM reply expected to be a JSON ARRAY of objects.

    Used by the general per-session Experience Distiller (Goal 2), whose prompt
    asks for a top-level JSON array of experience dicts. Tolerates the same
    wrappers as :func:`parse_distiller_json`: a bare array, a ```json fenced
    block, a plain fenced block, or an array embedded in surrounding prose.

    Returns the list of dict elements (non-dict elements are dropped). Returns
    ``[]`` when nothing parseable is found OR when the model legitimately emitted
    an empty array ``[]`` (the "nothing worth remembering" case).
    """
    if not raw or not isinstance(raw, str):
        return []
    text = raw.strip()
    ...
    # 2) Try the whole text as a JSON array.
    items = _try_load_array(text)
    if items is not None:
        return items

    # 3) Last resort: try each balanced [...] span until one parses.
    for span in _balanced_arrays(text):
        items = _try_load_array(span)
        if items is not None:
            return items

    # 4) Fall back to a single object — wrap it so a model that emitted one
    # experience without the enclosing array is not silently dropped.
