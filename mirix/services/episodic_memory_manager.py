...
class EpisodicMemoryManager:
    ...
    def get_episodic_memory_by_id(
        self, episodic_memory_id: str, timezone_str: str = None
    ...
    def get_most_recently_updated_event(
        self, organization_id: Optional[str] = None, timezone_str: str = None
    ) -> Optional[PydanticEpisodicEvent]:
        """
    ...
    def create_episodic_memory(
        self, episodic_memory: PydanticEpisodicEvent, actor: Optional[PydanticUser] = None
        ...
        # Set user_id from actor (use default user if no actor provided)
        if actor:
            episodic_memory_dict.setdefault("user_id", actor.id)
        else:
            # Use default user for backwards compatibility
            DEFAULT_USER_ID = "user-00000000-0000-4000-8000-000000000000"
            episodic_memory_dict.setdefault("user_id", DEFAULT_USER_ID)
        ...
        with self.session_maker() as session:
            ...
            episodic_memory_item.create(session, actor=actor)
            return episodic_memory_item.to_pydantic()
    ...
    def list_episodic_memory_around_timestamp(
        ...
        with self.session_maker() as session:
            ...
            query = select(EpisodicEvent).where(EpisodicEvent.occurred_at.between(start_time, end_time))
    ...
    def list_episodic_memory(
        ...
        with self.session_maker() as session:
            ...
            if query == "":
                ...
                # Filter by user_id when actor is provided for multi-user support
                if actor:
                    query_stmt = query_stmt.where(EpisodicEvent.organization_id == actor.organization_id)
                    query_stmt = query_stmt.where(EpisodicEvent.user_id == actor.id)
                if limit:
                ...
                if search_method == "embedding":
                    ...
                    if settings.mirix_pg_uri_no_default:
                        ...
                        return self._postgresql_fulltext_search(session, base_query, query, search_field, limit)
                    else:
                        # Fallback to in-memory BM25 for SQLite (legacy method)
                        # Load all candidate events (memory-intensive, kept for compatibility)
                        sqlite_query = select(EpisodicEvent)
                        # Filter by user_id when actor is provided for multi-user support
                        if actor:
                            sqlite_query = sqlite_query.where(EpisodicEvent.organization_id == actor.organization_id)
                            sqlite_query = sqlite_query.where(EpisodicEvent.user_id == actor.id)
                        result = session.execute(sqlite_query)
                    ...
                    fuzzy_query = select(EpisodicEvent)
                    # Filter by user_id when actor is provided for multi-user support
                    if actor:
                        fuzzy_query = fuzzy_query.where(EpisodicEvent.organization_id == actor.organization_id)
                        fuzzy_query = fuzzy_query.where(EpisodicEvent.user_id == actor.id)
                ...
                # Filter by user_id when actor is provided for multi-user support
                if actor:
                    main_query = main_query.where(EpisodicEvent.organization_id == actor.organization_id)
                    main_query = main_query.where(EpisodicEvent.user_id == actor.id)
    ...
    def _postgresql_fulltext_search(self, session, base_query, query_text, search_field, limit):
        ...
        if search_field == "summary":
            ...
            rank_sql = (
                f"ts_rank_cd(to_tsvector('english', coalesce(event_type, '')), to_tsquery('english', :tsquery), 32)"
            )
        else:
        ...
        try:
            ...
            results = list(session.execute(and_query_sql, {"tsquery": tsquery_string_and, "limit_val": limit or 50}))
        ...
        try:
            ...
            results = session.execute(or_query_sql, {"tsquery": tsquery_string_or, "limit_val": limit or 50})
