...
# Shared with the semantic write path: one definition of "same fact", so the two
# dedups cannot drift apart and start disagreeing about what a duplicate is.
from mirix.services.semantic_memory_manager import (  # noqa: E402
    _dedup_safe,
    _dedup_tokens,
)
...
class EpisodicMemoryManager:
    ...
    async def create_episodic_memory(
        ...
        # MIRIX_EPISODE_DEDUP: one real event must not become several rows.
        #
        # A counting question is answered by counting rows, so a restated event is an
        # extra count. Measured on conv-44, "how many times did Audrey and Andrew plan to
        # hike together" (gold 3, answered 5): the store holds
        #     2023-08-01  Andrew and Audrey plan a hiking trip with their dogs next month
        #     2023-08-01  Andrew and Audrey plan a hike with their dogs next month
        # Two turns three days apart, both saying "next month", both correctly resolved to
        # the same date, both written as separate events. occurred_at is already the
        # RESOLVED event date rather than the mention date, so the identity needed to
        # merge these is present and simply unused.
        #
        # Same discipline as the semantic dedup: append-only, never regenerate text, and
        # never merge across a differing quantity — "two dogs" and "three dogs" are
        # different facts however similar the sentence.
        _thresh = float(os.environ.get("MIRIX_EPISODE_DEDUP", "0") or 0)
        if _thresh > 0 and user_id:
            merged = await self._merge_into_same_episode(
                episodic_memory_dict, user_id, _thresh, actor)
    ...
    async def _merge_into_same_episode(self, data, user_id, thresh, actor):
        """Return the existing row describing the same episode, else None.

        Identity is (same resolved day) AND (near-identical wording). The date alone is far
        too coarse — a person does several distinct things a day — and the wording alone is
        far too coarse in the other direction, because a recurring activity is described
        the same way every time it happens. Requiring both is what keeps a weekly yoga
        class from collapsing into one event.
        """
        occ = data.get("occurred_at")
        if not isinstance(occ, datetime):
            return None
        incoming = f"{data.get('summary') or ''}"
        ...
        try:
            async with self.session_maker() as session:
                rows = (await session.execute(
                    ...
                    .where(EpisodicEvent.is_deleted.is_(False))
                    .where(func.date(EpisodicEvent.occurred_at) == day)
                )).scalars().all()
                best, best_j = None, 0.0
                for r in rows:
                    have = _dedup_tokens(r.summary or "")
                    if not have:
                        continue
                ...
                if best is None or best_j < thresh:
                    return None
                if not _dedup_safe(incoming, best.summary or ""):
                    return None
                new_details = str(data.get("details") or "").strip()
                if new_details and new_details not in str(best.details or ""):
                    best.details = (str(best.details or "") + "\n" + new_details).strip()
                    session.add(best)
                    await session.commit()
                    await session.refresh(best)
                logger.info("episode-dedup: folded %r into %s on %s (jaccard %.2f)",
                            incoming[:60], best.id, day, best_j)
                return best.to_pydantic()
        except Exception as e:  # noqa: BLE001
            logger.warning("episode-dedup failed, inserting normally: %s", e)
            return None
    ...
    async def insert_event(
        ...
        try:
            ...
            #   v5 → full LightRAG dual-graph (Episode + EpisodicEntity + EP_RELATES)
            #   v6 → lean entity index (V6Entity + V6_COOCCUR), no item nodes
