...
class GraphRetrieverDispatcher:
    ...
    async def retrieve(
        ...
        *,
        ...
        # v6/v7 short-circuit the v5 dual-graph pipeline entirely. They own
        # driver fetch, embedding, vector search, and PG enrichment.
        if settings.graph_version == "v6":
            from mirix.services.graph_retriever_v6 import V6Retriever

            return await V6Retriever().retrieve(
                query=query, user_id=user_id, agent_state=agent_state,
                top_k=item_top_k,
            )
        if settings.graph_version in ("v7", "v8"):
