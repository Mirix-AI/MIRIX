...
async def send_message_endpoint(request: MessageRequest):
    ...
    try:
        print(
            f"Starting agent.send_message (non-streaming) with: message='{request.message}', memorizing={request.memorizing}"
        )
...
async def send_streaming_message_endpoint(request: MessageRequest):
    ...
    def display_intermediate_message(message_type: str, message: str):
        ...
        message_queue.put({"type": "intermediate", "message_type": message_type, "content": message})
...
async def apply_persona_template(request: ApplyPersonaTemplateRequest):
    ...
    try:
        ...
        return UpdatePersonaResponse(
            success=True, message=f"Persona template '{request.persona_name}' applied successfully"
        )
    except Exception as e:
...
async def update_core_memory(request: UpdateCoreMemoryRequest):
    ...
    try:
        ...
        return UpdateCoreMemoryResponse(
            success=True, message=f"Core memory block '{request.label}' updated successfully"
        )
    except Exception as e:
...
async def set_model(request: SetModelRequest):
    ...
    try:
        ...
        return SetModelResponse(
            success=False, message=f"Error setting model: {str(e)}", missing_keys=[], model_requirements={}
        )
...
async def set_memory_model(request: SetModelRequest):
    ...
    try:
        ...
        return SetModelResponse(
            success=False, message=f"Error setting memory model: {str(e)}", missing_keys=[], model_requirements={}
        )
...
async def add_custom_model(request: AddCustomModelRequest):
    ...
    try:
        ...
        print(f"Traceback: {traceback.format_exc()}")
        return AddCustomModelResponse(success=False, message=f"Error adding custom model: {str(e)}")
...
async def set_screenshot_setting(request: ScreenshotSettingRequest):
    ...
    try:
        ...
        return ScreenshotSettingResponse(
            success=False, include_recent_screenshots=False, message=f"Error updating screenshot setting: {str(e)}"
        )
...
async def update_api_key(request: ApiKeyRequest):
    ...
    try:
        ...
        return ApiKeyUpdateResponse(success=result["success"], message=result["message"])
    except Exception as e:
        return ApiKeyUpdateResponse(success=False, message=f"Error updating API key: {str(e)}")
            ...
            credentials.append(
                {
                    ...
                    "content": (
                        "••••••••••••" if item.sensitivity == "high" else item.secret_value
                    ),  # Always mask the actual content
                }
            )
...
async def clear_conversation_history():
    ...
    try:
        ...
        current_messages = agent.client.server.agent_manager.get_in_context_messages(
            agent_id=agent.agent_states.agent_state.id, actor=agent.client.user
        )
...
async def trigger_reflexion(request: ReflexionRequest):
    ...
    try:
        ...
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, _run_reflexion_process, agent)  # Use default ThreadPoolExecutor
        ...
        return ReflexionResponse(success=result["success"], message=result["message"], processing_time=processing_time)
