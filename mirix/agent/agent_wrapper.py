...
from mirix.agent.app_constants import (
    ...
    WITH_BACKGROUND_AGENT,
    WITH_REFLEXION_AGENT,
)
...
from ..voice_utils import convert_base64_to_audio_segment, process_voice_files
from .app_utils import encode_image, encode_image_from_pil
...
class AgentWrapper:
        ...
        if len(self.client.list_agents()) > 0:
            ...
            for agent_state in all_agent_states:
                ...
                system_prompt = (
                    ...
                    else gpt_system.get_system_text("screen_monitor/" + agent_state.name)
                )

                self.client.server.agent_manager.update_agent_tools_and_system_prompts(
                    agent_id=agent_state.id, actor=self.client.user, system_prompt=system_prompt
                )
            ...
            core_memory = ChatMemory(
                human="", persona="You are a helpful personal assitant who can help the user remember things."
            )
            ...
            for config in AGENT_CONFIGS:
                if config["name"] == "chat_agent":
                    ...
                    agent_state = self.client.create_agent(
                        ...
                        memory=core_memory,
                        system=(
                            gpt_system.get_system_text("screen_monitor/" + config["name"])
    ...
    def _initialize_gemini_components(self) -> bool:
        ...
        try:
            ...
            self.uri_to_create_time = {
                file.uri: {"create_time": file.create_time, "filename": file.name} for file in existing_files
            }
    ...
    def _process_existing_uploaded_files(self):
        ...
        for mapping in uploaded_mappings:
            ...
            self.temp_message_accumulator.temporary_messages.append(
                (mapping.timestamp, {"image_uris": [file_ref], "audio_segments": None, "message": None})
            )
            count += 1
    ...
    def set_persona(self, persona_name):
        ...
        self.client.server.block_manager.update_block(
            block_id=persona_block.id, block_update=BlockUpdate(value=persona_text), actor=self.client.user
        )
    ...
    def set_model(self, model_name: str, custom_agent_config: dict = None) -> dict:
        ...
        try:
            ...
            self.client.server.agent_manager.update_llm_config(
                agent_id=self.agent_states.agent_state.id, llm_config=llm_config, actor=self.client.user
            )
            # set the model for reflexion agent
            self.client.server.agent_manager.update_llm_config(
                agent_id=self.agent_states.reflexion_agent_state.id, llm_config=llm_config, actor=self.client.user
            )
    ...
    def set_memory_model(self, new_model, custom_agent_config: dict = None):
        ...
        if new_model not in ALLOWED_MEMORY_MODELS:
            ...
            if custom_agent_config is not None:
                ...
                return {
                    ...
                    "missing_keys": [],
                    "model_requirements": {"current_model": new_model, "required_keys": []},
                }
            ...
            if new_model in GEMINI_MODELS:
                ...
                if not self.is_gemini_client_initialized():
                    return {
                        ...
                        "model_requirements": {"current_model": new_model, "required_keys": ["GEMINI_API_KEY"]},
                    }
            ...
            # Check if it's an OpenAI model
                ...
                if not has_openai_key:
                    return {
                        ...
                        "model_requirements": {"current_model": new_model, "required_keys": ["OPENAI_API_KEY"]},
                    }
        ...
        for agent_state in self.client.list_agents():
            if agent_state.name in memory_agent_names:
                self.client.server.agent_manager.update_llm_config(
                    agent_id=agent_state.id, llm_config=llm_config, actor=self.client.user
                )
        ...
        return {
            ...
            "missing_keys": [],
            "model_requirements": {"current_model": new_model, "required_keys": required_keys},
        }
    ...
    def update_core_memory_persona(self, text: str):
        ...
        """
        self.client.update_agent_memory_block(agent_id=self.agent_states.agent_state.id, label="persona", value=text)

    def update_core_memory(self, text: str, label: str):
        """
        Update the core memory with the given text and label
        """
        self.client.update_agent_memory_block(agent_id=self.agent_states.agent_state.id, label=label, value=text)
    ...
    def clear_old_screenshots(self):
        ...
        if len(self.uri_to_create_time) > MAXIMUM_NUM_IMAGES_IN_CLOUD:
            ...
            files_to_delete = sorted(self.uri_to_create_time.items(), key=lambda x: x[1]["create_time"])[
            ...
            if self.google_client is not None:
                threading.Thread(
                    target=self.delete_files, args=([x for x in file_names_to_delete], self.google_client)
                ).start()
            else:
...
Please perform this analysis and create new memories as appropriate. Provide a detailed report of patterns found and new memories created."""
    ...
    def _analyze_core_redundancy(self, core_blocks):
        ...
        redundancy_info = {"total_blocks": len(core_blocks), "overlapping_content": [], "recommendations": []}
    ...
    def _get_memory_content(self, memory, memory_type):
        ...
        try:
            if memory_type == "episodic":
                ...
                return (
                    f"{getattr(memory, 'name', '')} {getattr(memory, 'summary', '')} {getattr(memory, 'details', '')}"
                )
            elif memory_type == "procedural":
                return f"{getattr(memory, 'summary', '')} {' '.join(getattr(memory, 'steps', []))}"
            elif memory_type == "resource":
                return (
                    f"{getattr(memory, 'title', '')} {getattr(memory, 'summary', '')} {getattr(memory, 'content', '')}"
                )
    ...
    def send_message(
        ...
        # Handle user context switching for multi-user support
        original_user = None
        if user_id is not None:
            # Store original user context to restore later
            original_user = self.client.user
            # Switch to the specified user context
            try:
                switched_user = self.client.server.user_manager.get_user_or_default(user_id)
                self.client.user = switched_user
            except Exception as e:
                self.logger.warning(f"Failed to switch to user {user_id}, using default user: {e}")
        ...
        if self.model_name in GEMINI_MODELS and not self.is_gemini_client_initialized():
            if images is not None or image_uris is not None or voice_files is not None:
                ...
                if message is None:
                    ...
                    return (
                        "Error: Gemini API key required for image/voice features. Please configure it in the settings."
                    )
            ...
            if self.include_recent_screenshots:
                ...
                if len(most_recent_images) > 0:
                    ...
                    extra_messages.append(
                        {"type": "text", "text": f"Additional images (screenshots) from the system start here:"}
                    )
                    ...
                    for idx, (timestamp, file_ref, source) in enumerate(most_recent_images):
                        ...
                        if hasattr(file_ref, "uri"):
                            ...
                            extra_messages.append(
                                {"type": "text", "text": f"Timestamp: {timestamp}; Image Index {idx}" + source_text}
                            )
                            extra_messages.append(
                    ...
                    extra_messages.append(
                        {"type": "text", "text": f"Additional images (screenshots) from the system end here."}
                    )
    ...
    def export_memories_to_csv(self, csv_file_path: str, include_embeddings: bool = False) -> dict:
        ...
        try:
            ...
            try:
                episodic_memories = self.client.server.episodic_memory_manager.list_episodic_memory(
                    agent_state=self.agent_states.episodic_memory_agent_state, limit=None
                )
            ...
            try:
                semantic_memories = self.client.server.semantic_memory_manager.list_semantic_items(
                    agent_state=self.agent_states.semantic_memory_agent_state, limit=None
                )
            ...
            try:
                procedural_memories = self.client.server.procedural_memory_manager.list_procedures(
                    agent_state=self.agent_states.procedural_memory_agent_state, limit=None
                )
            ...
            try:
                resource_memories = self.client.server.resource_memory_manager.list_resources(
                    agent_state=self.agent_states.resource_memory_agent_state, limit=None
                )
            ...
            try:
                knowledge_vault_items = self.client.server.knowledge_vault_manager.list_knowledge(
                    agent_state=self.agent_states.knowledge_vault_agent_state, limit=None
                )
    ...
    def export_memories_to_excel(
        self, file_path: str, memory_types: list = None, include_embeddings: bool = False
    ) -> dict:
        """
        ...
        result = {"success": False, "message": "", "exported_counts": {}, "total_exported": 0, "file_path": file_path}
