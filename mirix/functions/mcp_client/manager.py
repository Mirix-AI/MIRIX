...
class MCPClientManager:
    ...
    async def _ensure_loaded(self) -> None:
        """Load persistent connections on first use"""
        if self._load_started:
            return
        self._load_started = True
        await self._load_persistent_connections()
    ...
    async def _add_server_without_persistence(
        self,
        server_config: BaseServerConfig,
        environment_variables: Optional[Dict[str, str]] = None,
    ) -> bool:
        """Add a server configuration and connect without saving to disk (used for restoration)"""
        try:
            # Resolve any templated environment variables
            server_config.resolve_environment_variables(environment_variables)
    ...
    async def execute_tool_by_name(self, tool_name: str, tool_args: Dict[str, Any]) -> Tuple[str, bool]:
        ...
        if result:
            ...
            return await self.execute_tool(server_name, tool_name, tool_args)
        else:
    ...
    async def _load_persistent_connections(self) -> None:
        ...
        try:
            ...
            for server_name, config_data in configs_data.items():
                try:
                    ...
                    if await self._add_server_without_persistence(config):
            ...
            logger.error("Failed to load persistent MCP server configurations: %s", str(e))
...
async def get_mcp_client_manager() -> MCPClientManager:
    """Get the singleton MCP client manager instance (async)"""
    global _mcp_manager
    if _mcp_manager is None:
        _mcp_manager = MCPClientManager()
        await _mcp_manager._load_persistent_connections()
        _mcp_manager._load_started = True
    return _mcp_manager
