...
class MetaClawConfig:
    ...
    # MIRIX generic memory harness; default preserves native MetaClaw behavior.
    # "raw_transcript" (default): the legacy every-N-turns batch path
    #   (`_evolve_skills_for_session` via `_session_turns`) for native MetaClaw.
    # "generic_memory": message-by-message path — per-round proxy callback to
    #   MIRIX production /memory/add_sync via MirixGenericMemoryAdapter.
    skill_evolution_mode: str = "raw_transcript"
    # Retained config shape; MIRIX's actual procedural cadence lives server-side.
