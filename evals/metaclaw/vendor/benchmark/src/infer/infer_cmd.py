...
async def _start_work_gateway(
    ...
    with open(log_path, "w", encoding="utf-8") as log_fh:
        proc = await asyncio.create_subprocess_exec(
            ...
            "--port", str(port),
            "--allow-unconfigured",
            # Local fork: openclaw 2026.5.18 generates a runtime auth token
            # when config is missing one; the in-process agent client doesn't
            # know that token and fails with "unauthorized". Forcing
            # --auth none lets the bench's localhost-only loopback gateway
            # run without auth (which is what --allow-unconfigured implies
            # in spirit). See PRD known-deviations / gating tripwire.
            "--auth", "none",
            env=env,
            stdout=log_fh,
            stderr=log_fh,
...
async def _run_one_test(
    ...
    async with outer_semaphore:
        ...
        # Local fork: bump from default 10s to 60s — openclaw gateway boot
