...
def measure_memory_size(sample_id: str) -> Dict:
    ...
    pg_bin = os.environ.get("PSQL_BIN", "/usr/local/opt/postgresql@17/bin/psql")
    pg_host = os.environ.get("MIRIX_PG_HOST", "localhost")
    pg_port = os.environ.get("MIRIX_PG_PORT", "5432")
    pg_user = os.environ.get("MIRIX_PG_USER", "mirix")
    pg_db = os.environ.get("MIRIX_PG_DB", "mirix")
    pg_password = os.environ.get("MIRIX_PG_PASSWORD", "mirix")
    import subprocess

    def _pg(sql: str) -> str:
        try:
            out = subprocess.run(
                [pg_bin, "-h", pg_host, "-p", pg_port, "-U", pg_user, "-d", pg_db, "-tAc", sql],
